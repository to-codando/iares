...
`,result:""}),"addGlobalRule"),u=n(t=>{let o=(t.match(/{/g)||[]).length,d=(t.match(/}/g)||[]).length;return o-d},"countBlocks"),i=n((t,o)=>{if(t.insideBlock+=u(o),t.insideBlock===0&&l.test(o)){let{globalRules:S,result:$}=m(o,t.globalRules);return{...t,globalRules:S,result:t.result+$}}if(t.globalRules){let{globalRules:S,result:$}=c(t.globalRules,t.result,r),E=a(o,"");return{...t,globalRules:S,result:$+E.result}}let{globalRules:d,result:x}=a(o,t.result);return{...t,globalRules:d,result:x}},"processLine"),{result:y,globalRules:p}=e.reduce(i,g);return p?`${y}${r} {
${p}}
...
`);return`@media ${c.trim()} {
${u}
}`})},"wrapLooseRulesInsideMediaQuery"),w=n(({style:s,selector:r})=>{let e=/\.(\w+)/g;return s.replace(e,`.${r}_$1`)},"applyClassNameScope"),R=n((s,r)=>{let e=s,l=`.${r}`;return e=w({style:e,selector:r}),e=M({style:e,selector:l}),e=P({style:e,selector:l}),e},"transformStyle");var L=new Map,T=n((s,r=()=>{})=>(e,...l)=>{let g=e.reduce((i,y,p)=>`${i}${y}${l[p]!==void 0?l[p]:""}`,""),c=L.get(g);if(c!==void 0)return c;let a=f(g,s),m=R(g,`${a}`),u=h(`${a}`);return r({hashId:a,scopedStyle:m,styleElement:u}),u.innerHTML.includes(m)||(u.innerHTML+=m),L.set(g,a),a},"css");export{f as createHash,h as createStyleElement,T as css,R as transformStyle};
